...
(comment
  (do
    ...
    (require '[datahike.api :as da])

;;    (prof/serve-files 8080)

    (let [cfg        {:store              {:backend :file :path "/tmp/file-example"}
                      :schema-flexibility :read}
          #_{:store              {:backend :mem :id "/tmp/file-example"}
             :schema-flexibility :read}
          _          (da/delete-database cfg)
          _          (da/create-database cfg)
          conn       (da/connect cfg)
          all-datoms (map
                      #(vector :db/add (+ 1 %1) :aka 1 %1)
                      (range 1000))]
      #_(time (doall (doseq [datom all-datoms]
                       (da/transact conn [datom]))))

      (prof/profile (doall (doseq [datom all-datoms]
                             (da/transact conn [datom]))))
